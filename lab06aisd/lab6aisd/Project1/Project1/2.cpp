...
int main() {
    ...
    // Создаем две хеш-таблицы с размерами 16 и 32
    HashTable table16(16);
    HashTable table32(32);

    // Заполняем таблицы случайными числами от 0 до 99
    for (int i = 0; i < 100; ++i) {
        table16.insert(i, i);
        table32.insert(i, i);
    }

    // Замеряем время поиска элемента в таблице размера 16
    ...
    // Замеряем время поиска элемента в таблице размера 32
